...
//登记体检信息 
void registerPhysicalExamination(){
...
//修改体检信息 
void modifyPhysicalExamination(){
	char studentid[20];
	char year[5];
	PhysicalExamination pe;
...
//删除体检信息 
void deletePhysicalExamination(){
	char studentid[20];
	char year[5];
	PhysicalExamination pe;
	FILE *file = fopen("physicalexamination.txt","r");
...
void statisticsPhysicalExamination(){
	...
	//计算总增长量
	float w_total_growth = weight[count - 1] - weight[0];
    float c_total_growth = chest[count - 1] - chest[0];
    //计算时间段
    int start_year = year[0],end_year = year[count - 1];
	...
	//计算平均年增长值
	float w_average_growth_per_year = w_sum_growth/total_years;
	float c_average_growth_per_year = c_sum_growth/total_years;
	//计算年增长率
	float w_growth_rate =  (w_total_growth/total_years)*100;
